...
> 本文档列出 NewLife.NovaDb 的内置 SQL 函数，包含优先级、签名、返回类型、NULL/错误语义。
> 
...
| `TRUNCATE` | — | `TRUNCATE(n, decimals)` | 截断（不四舍五入） |
...
### 4.1 聚合
`STDDEV(expr)` — 样本标准差 | `VARIANCE(expr)` — 样本方差

### 4.2 字符串
`REVERSE(s)` | `LPAD(s, len, pad)` | `RPAD(s, len, pad)`

### 4.3 数值
`PI()` | `EXP(n)` | `LOG(n)` | `LOG10(n)`

### 4.4 日期时间
`LAST_DAY(dt)` | `TIMESTAMPDIFF(unit, dt1, dt2)` | `TIMESTAMPADD(unit, n, dt)`
