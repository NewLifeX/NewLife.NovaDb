...
1. **使用批量操作**：通过 `SetAll`/`GetAll` 合并多次操作为一次 RPC，可将吞吐量提升 **26-66 倍**。详见 [NovaCache 批量操作性能测试报告](NovaCache批量操作性能测试报告.md)
...
6. **批量数据导入**：使用 `SetAll` 批量写入（每批 100-1000 条）可达 10 万+ ops/s
